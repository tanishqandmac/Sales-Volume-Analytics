...
def orders_create(request):
    try:
        userObject = UserDatabase.objects.get(
            domainName=str((request.webhook_domain)).split(".")[0]
        )
...
def app_uninstalled(request):
    try:
        ...
        UserDatabase.objects.filter(
            domainName=str(data["domain"]).split(".")[0]
        ).delete()
        print(str(data["domain"]).split(".")[0])
        print(str(data["domain"]))
        user = AuthAppShopUser.objects.filter(
            myshopify_domain=str(data["domain"])
        ).order_by("-id")[0]
        user.delete()
        return HttpResponse("200")
    except Exception:
...
def index(request, *args, **kwargs):
    with request.user.session:
        try:
            obj, flag = UserDatabase.objects.get_or_create(
                domainName=str(request.user).split(".")[0]
            )
            print(obj)
            print(flag)
            if obj.flag == 1:
                ...
                if date == "":
                    ...
                    productsList = ProductsDatabase.objects.filter(
                        sno=userObject, createdAt__range=(dates[0], dates[1])
                    )
                ...
                if sync == "True":
                    return render(
                        request,
                        "core/report.html",
                        {"Products": pList, "Summary": Summary, "Sync": "True"},
                    )
                else:
                    return render(
                        request,
                        "core/report.html",
                        {"Products": pList, "Summary": Summary},
                    )
            else:
                print("Index Billing")
...
def webhookCreation(domain_name, user_token):
    ...
    headers = {"Content-Type": "application/json", "X-Shopify-Access-Token": user_token}

    # Graphql Query for Checking Webhook Existence
    r = requests.post(
        GRAPHQL_URL_QUERY.format(domain_name),
        headers=graphql_headers,
        data=GRAPHQL_WEBHOOK_CHECK_QUERY,
    )
    response = r.json()
...
def synchronisation(domain_name, user_token):
    ...
    userObject = UserDatabase.objects.filter(
        domainName=str(domain_name).split(".")[0]
    ).delete()
    ...
    while GrossSales[0] != 0:
        if GrossSales[0] == 1:
            dataH = GRAPHQL_PRODUCT_FETCH_QUERY.format(
                ", after:" + '"' + GrossSales[1] + '"'
            )
            response = requests.post(
                GRAPHQL_URL_QUERY.format(domain_name),
...
def GrossSalesCal(userObject, response, customProductsList):
    try:
        ...
        if response["data"]["orders"]["pageInfo"]["hasNextPage"] == True:
            return [
                1,
                response["data"]["orders"]["edges"][-1]["cursor"],
                customProductsList,
            ]
        else:
...
def ProductsDictMaker(userObject, productsList):
    plist = []
    distinctProducts = productsList.values_list(
        "sku", "productName", "vendor"
    ).distinct()
...
def activation(request, *args, **kwargs):
    with request.user.session:
        ...
        response = requests.post(
            url.format(domain=request.user, charge=charge), headers=headers
        )
        try:
            if response.json()["application_charge"]["status"] == "active":
                obj = UserDatabase.objects.get(
                    domainName=str(request.user).split(".")[0]
                )
                obj.flag = 1
                obj.save()
...
def billing(request, *args, **kwargs):
    with request.user.session:
        ...
        response = requests.post(url.format(domain_name), headers=headers, json=json)
        try:
            if response.json()["application_charge"]["status"] == "pending":
                return redirect(
                    response.json()["application_charge"]["confirmation_url"]
                )
            else:
                return redirect("core:index")
        except:
